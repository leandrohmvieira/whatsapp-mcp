...
func NewMessageStore() (*MessageStore, error) {
	...
	// Migration for databases created before media DirectPath capture: add the
	// direct_path column if missing (the "duplicate column" error is expected & ignored).
...
func downloadMedia(client *whatsmeow.Client, messageStore *MessageStore, messageID, chatJID string) (bool, string, string, string, error) {
	...
	// Generate a local path for the file. The bridge derives `filename` from a
	// timestamp, so distinct messages can share one name (many history-synced
	// media land in the same second). Prefix the message ID so every message maps
	// to its own file — otherwise the "already downloaded" shortcut below would
	// hand back the wrong media for a colliding name.
	localPath = fmt.Sprintf("%s/%s_%s", chatDir, sanitizeFilenameComponent(messageID), filename)
	...
	// Prefer the real DirectPath captured from the message (whatsmeow builds the
	// authenticated media-conn URL from it, and the query prefix must be preserved).
	// Fall back to deriving it from the stored URL for legacy rows synced before
	// direct_path was captured.
	if directPath == "" {
...
// sanitizeFilenameComponent keeps only filesystem-safe characters so a message
// ID can be used as a filename prefix.
...
func extractDirectPathFromURL(url string) string {
	...
	// Keep the query string. whatsmeow's DownloadMediaWithPath builds the media-conn
	// URL as "https://<host><directPath>&hash=...&mms-type=...", so the direct path
	// must already carry its "?ccb=...&_nc_sid=..." prefix for the result to be a
	// well-formed URL. Stripping it (the old behaviour) produced a malformed URL and
	// WhatsApp answered 403. Used only as a fallback for legacy rows that predate
	// storing the message's real DirectPath.
	return "/" + parts[1]
}
